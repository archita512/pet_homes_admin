        ...
        <div class="k-table-container d-flex flex-column justify-content-between">
          <div class="table-responsive">
            <table id="tbl_cat" class="table table-striped table-hover">
              ...
              <tbody>
                <?php
                  $query = mysqli_query($cnn,"SELECT ad.*,p.image FROM `pet_return` AS ad JOIN pets AS p ON ad.pet_id = p.id ORDER BY ad.id DESC");
                  $cnt = 1;
...
deleteCategoryModal.addEventListener('show.bs.modal', function (event) {
    ...
    const message = `Are you sure you want to delete Pets Return?`;
    ...
    deleteButton.onclick = function() {
        fetch(`crud.php?what=delete_pet_return`, {
            method: 'POST',
            headers: {
            ...
            $('#submitPayment').on('click', function () {
              ...
              if (method === "") {
                var toastHTML = `
                        ...
                        $.ajax({
                    url: 'crud.php?what=update_payment_return',
                    method: 'POST',
                    data: {
                      id: id,
                      status: method
